...
export function LeafletMap({
  customerCoords = [12.9352, 77.6245],
  workerCoords = null,
  height = '220px',
  zoom = 14
}) {
  return (
    <div style={{ height, width: '100%' }} className="rounded-2xl overflow-hidden border border-slate-700">
      <MapContainer
      ...
      >
        ...
        {customerCoords && (
          <Marker position={customerCoords} icon={customerIcon}>
            <Popup><span className="font-bold text-xs text-slate-900">Customer Location</span></Popup>
          </Marker>
        )}

        {workerCoords && (
          <Marker position={workerCoords} icon={workerIcon}>
            <Popup><span className="font-bold text-xs text-blue-600">Your Current GPS</span></Popup>
          </Marker>
        )}

        {customerCoords && workerCoords && (
          <Polyline positions={[customerCoords, workerCoords]} color="#3b82f6" dashArray="6, 8" weight={3} />
        )}
      </MapContainer>
    </div>
  );
